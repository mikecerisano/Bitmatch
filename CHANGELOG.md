...
- Accessibility: Closed out the remaining items from the 2026-09-25 audit (see `docs/audits/2026-09-25-accessibility.md`, "Status" section, for the full list). A checksum mismatch is now shown as a failure (red, distinct icon), not a minor warning, on the Mac results table and everywhere else that reads `ResultStatusPresentation` (Promise 2). The Mac live verified/issues counts have distinct icons and a spoken label each, not just a colored dot. Every Mac result row reads as one VoiceOver stop ("file, status, size, destination") instead of four or five. The Mac results filter toggle has a stable VoiceOver label. Mac Photographer views (`DesignSystem.Typography`) now scale with Dynamic Type instead of using fixed point sizes. The camera-label position picker, custom-label field, and remote-destination fields and menu have real accessibility labels. Cancel and drop-rejection toasts on Mac and iPad are announced to VoiceOver, not just shown briefly on screen.
...
- Reports and results: each file is labelled with the backup it was written to. For a backup folder outside /Volumes (for example on the startup disk), files were labelled with a folder two levels above them, and the outcome screen could list the source folder as a backup.
